...
export const Navbar = () => {
  ...
  return (
    <nav className="navbar navbar-light bg-light mb-3 sticky-top nav">
      ...
      <div className="nav-item dropdown">
        ...
        <ul className="dropdown-menu menu">
          {store.favoritos.map(item => (
            <li className="dropdown-item me-4"> {item.person ?.name}{item.planet ?.name}{item.vehicle?.name}{item.starship ?.name}<span onClick={() => actions.eliminarFavorito(item)}><span className="ms-2 btn btn-danger float-end"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash3" viewBox="0 0 16 16">
      ...
      {store.auth && <button onClick={()=>actions.logout(navigate)} className="btn btn-primary mx-5">Log Out</button>}
    </nav>
  );
};
